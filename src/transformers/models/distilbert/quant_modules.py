...
class QuantAct(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            if torch.isinf(x_act).any():
                print(f"==== IS-INF ==== {np.where(x_act.detach().clone().cpu().numpy() == float('inf'))}")
            if x_act.isnan().sum() > 0:
                print(f"==== IS-NAN ==== {np.where(np.isnan(x_act.detach().clone().cpu().numpy()))}")
...
class IntLayerNorm(nn.Module):
    ...
    def forward(self, x, scaling_factor=None):
        ...
        x_int = x / scaling_factor
        if x_int.isnan().any() or scaling_factor.isnan().any():
            print("======== X_INT or Scaling factor is nan at line 508 ==========")
        if torch.isinf(x_int).any() or torch.isinf(scaling_factor).any():
            print("======== X_INT or Scaling factor is inf at line 510 ==========")
        ...
        if x.isnan().any() or scaling_factor.isnan().any():
            print("======== X or Scaling factor is nan at line 543 ==========")
        if torch.isinf(x).any() or torch.isinf(scaling_factor).any():
            print("======== X_INT or Scaling factor is inf at line 545 ==========")
